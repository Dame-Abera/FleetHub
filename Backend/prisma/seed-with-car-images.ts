...
// Car data with proper image fetching
const sampleCars = [
  {
    name: 'BMW X5',
    brand: 'BMW',
    ...
    // These will be replaced with actual car images
    images: [
...
// Function to fetch car images from CarsXE API
async function fetchCarImages(make: string, model: string, year: number): Promise<string[]> {
  try {
    // Note: You'll need to get a free API key from https://api.carsxe.com/
    const apiKey = process.env.CARSXE_API_KEY || 'YOUR_CARSXE_API_KEY';
    
    if (apiKey === 'YOUR_CARSXE_API_KEY') {
      console.log(`⚠️  No CarsXE API key found for ${make} ${model} ${year}. Using fallback images.`);
      return [
        'https://images.unsplash.com/photo-1555215695-3004980ad54e?w=800&h=600&fit=crop&auto=format',
        'https://images.unsplash.com/photo-1606664515524-ed2f786a0bd6?w=800&h=600&fit=crop&auto=format'
      ];
    ...
    const response = await fetch(
      `https://api.carsxe.com/images?key=${apiKey}&make=${make}&model=${model}&year=${year}&format=json`
    );
    
    const data = await response.json();
    
    if (data.success && data.images && data.images.length > 0) {
      console.log(`✅ Found ${data.images.length} images for ${make} ${model} ${year}`);
      return data.images.slice(0, 2).map((img: any) => img.link);
    } else {
      console.log(`❌ No images found for ${make} ${model} ${year}. Using fallback images.`);
      return [
        'https://images.unsplash.com/photo-1555215695-3004980ad54e?w=800&h=600&fit=crop&auto=format',
        'https://images.unsplash.com/photo-1606664515524-ed2f786a0bd6?w=800&h=600&fit=crop&auto=format'
      ];
    ...
    console.error(`❌ Error fetching images for ${make} ${model} ${year}:`, error);
    return [
      'https://images.unsplash.com/photo-1555215695-3004980ad54e?w=800&h=600&fit=crop&auto=format',
      'https://images.unsplash.com/photo-1606664515524-ed2f786a0bd6?w=800&h=600&fit=crop&auto=format'
    ];
...
async function main() {
  console.log('🌱 Starting database seeding with car-specific images...');
  ...
  // Create sample cars with proper images
  const createdCars = [];
  for (const carData of sampleCars) {
    console.log(`🔄 Fetching images for ${carData.brand} ${carData.name} ${carData.year}...`);
    
    // Fetch proper car images
    const carImages = await fetchCarImages(carData.brand, carData.name, carData.year);
    
    const car = await prisma.car.create({
      data: {
        ...carData,
        images: carImages, // Use the fetched images
  ...
  console.log('✅ Cars created with proper images:', createdCars.length);
  ...
  console.log('\n🚗 Car Images:');
  console.log('Each car now has proper model-specific images fetched from CarsXE API!');
  console.log('To get actual car images, add your CarsXE API key to .env file:');
  console.log('CARSXE_API_KEY=your_api_key_here');
}
